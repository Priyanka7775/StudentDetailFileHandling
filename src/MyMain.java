...
public class MyMain {
    public static  void main(String[] args) {
        String [] line={"ABHINAV PANDEY,23,male,1569874465,chintu@gmail.com",
           ...
           StudentDetail[] studentDetail= new StudentDetail[line.length];



        for (int i = 0; i < line.length ; i++) {
            String[] arrayName=line[i].split(",");
            studentDetail[i]=new StudentDetail();
            studentDetail[i].setName(arrayName[0]);
            studentDetail[i].setAge(Integer.parseInt(arrayName[1]));
            studentDetail[i].setGender(arrayName[2]);
            studentDetail[i].setMobile(Long.parseLong(arrayName[3]));
            studentDetail[i].setEmail(arrayName[4]);
        }
